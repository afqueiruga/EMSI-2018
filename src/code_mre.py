...
class Thermomechanics():
	def __init__(self, mesh, cell_ids, facet_ids):
		...
		self.mesh_sub = SubMesh2(self.mesh_glob, cell_ids, (cfg['gel'],) )
		...
		self.traction= Constant([0,0,1E5])
		...
		self.phi_sub = Function(self.Scalar)
		...
		self.mu_mat = AssignMaterialCoefficients(self.mesh_sub, self.sub_cells, [mu_a, mu_e], mat_marking)
		self.alp_mat = AssignMaterialCoefficients(self.mesh_sub, self.sub_cells, [alp_a, alp_e], mat_marking)
		self.m_mat = AssignMaterialCoefficients(self.mesh_sub, self.sub_cells, [m_a, m_e], mat_marking)
		self.n_mat = AssignMaterialCoefficients(self.mesh_sub, self.sub_cells, [n_a, n_e], mat_marking)
		self.q_mat = AssignMaterialCoefficients(self.mesh_sub, self.sub_cells, [q_a, q_e], mat_marking)
		self.r_mat = AssignMaterialCoefficients(self.mesh_sub, self.sub_cells, [r_a, r_e], mat_marking)

		self.Ttilde = VoigtToTensorRank3() 

		self.Rtilde = VoigtToTensorRank2()

		eps_rel_el11 = AssignMaterialCoefficients(self.mesh_sub, self.sub_cells, [eps_rel_el_a, eps_rel_el_e], mat_marking)
		eps_rel_el33 = AssignMaterialCoefficients(self.mesh_sub, self.sub_cells, [eps_rel_el_a, eps_rel_el_e], mat_marking)
		eps_rel_el = VoigtToTensorRank2(A11=eps_rel_el11, A22=eps_rel_el11, A33=eps_rel_el33)
		self.chi_el = eps_rel_el - delta

		mu_rel_mag11 = AssignMaterialCoefficients(self.mesh_sub, self.sub_cells, [mu_rel_ma_a, mu_rel_ma_e], mat_marking)
		...
		alpha11 = AssignMaterialCoefficients(self.mesh_sub, self.sub_cells, [alpha_a, alpha_e], mat_marking)
		alpha33 = AssignMaterialCoefficients(self.mesh_sub, self.sub_cells, [alpha_a, alpha_e], mat_marking)
		self.alpha = VoigtToTensorRank2(A11=alpha11, A22=alpha11, A33=alpha33)

		self.varsigma = AssignMaterialCoefficients(self.mesh_sub, self.sub_cells, [varsigma_a, varsigma_e], mat_marking)
		self.kappa    = AssignMaterialCoefficients(self.mesh_sub, self.sub_cells, [kappa_a, kappa_e], mat_marking)
		self.pi       = AssignMaterialCoefficients(self.mesh_sub, self.sub_cells, [pi_a, pi_e], mat_marking)
		self.c        = AssignMaterialCoefficients(self.mesh_sub, self.sub_cells, [c_a, c_e], mat_marking)
		self.rho_0    = AssignMaterialCoefficients(self.mesh_sub, self.sub_cells, [rho_0_a, rho_0_e], mat_marking)
		self.v_0      = 1./self.rho_0

	def free_energy(self, F, B):
		i,j = indices(2)
		CC = as_tensor( F[j,i]*F[j,k] , [i,k])
		I1 = as_tensor( CC[i,i] , [])
		I2 = as_tensor( 1./2.*(I1**2 - CC[i,j]*CC[j,i]) , [])
		I4 = as_tensor( B[i]*B[i] , [])
		I6 = as_tensor( CC[i,j]*B[j] * CC[i,k]*B[k] , [])
		mu_mat, alp_mat, m_mat, n_mat, q_mat, r_mat = self.mu_mat, self.alp_mat, self.m_mat, self.n_mat, self.q_mat, self.r_mat
		free = mu_mat/4.*( 1.+alp_mat*tanh(I4/m_mat) )*( (1.+n_mat)*(I1-3.) + (1.-n_mat)*(I2-3.) ) + q_mat*I4 + r_mat*I6
		return free

	def loading_force(self):
		i,j,k = indices(3)
		E, B = self.E_sub, self.B_sub
		u, T = split(self.unkn)
		Ttilde,alpha,Rtilde,chi_el,chi_mag,mu_mag,varsigma,kappa,pi,c,rho_0,v_0 = self.Ttilde,self.alpha,self.Rtilde,self.chi_el,self.chi_mag,self.mu_mag,self.varsigma,self.kappa,self.pi,self.c,self.rho_0,self.v_0
		F = as_tensor( u[i].dx(j) + delta[i,j] , (i,j))
		F_var = variable(F)
		B_var = variable(B)
		free_e = self.free_energy(F_var,B_var)
		NN_var = diff(free_e,F_var)
		C = diff(NN_var,F_var)
		Stilde = -diff(NN_var,B_var)

		P = as_tensor( 1./det(F)*Ttilde[i,k,l]*(-alpha[k,l]*(T-Tref) + u[k].dx(l) ) + eps_0*chi_el[i,k]*E[k] + (2.-1./det(F))*Rtilde[k,i]*B[k] , (i,))
		MM = as_tensor( 1./det(F)*Stilde[i,k,l]*(-alpha[k,l]*(T-Tref) + u[k].dx(l) ) + Rtilde[i,k]*E[k] + (2.-1./det(F))*inv(mu_mag)[i,j]*chi_mag[j,k]*B[k] , (i,))
		...
		z_direction = Constant((0.,0.,1.))
		force = assemble( det(F)*inv(F)[k,i]*N[k]*sigma[i,j]*z_direction[j]*self.dA(cfg['load']) )
		return force
	...
	def make_forms(self):
		...
		F_T = ( rho_0*(eta-eta0)*del_T - Dt*Q[i]/T*del_T.dx(i) - Dt*rho_0*r/T*del_T + Dt*Q[j]/T/T*T.dx(j)*del_T - Dt*det(F)/T*EE[i]*JJ_fr[i]*del_T )*dV + Dt*h_interface*(T-Tref)/T*del_T*(dA(cfg['interface'])+dA(cfg['clamp'])+dA(cfg['load']))
...
class Electromagnetism():
	...
	def __init__(self, mesh, cell_ids, facet_ids):
		...
		bc = []
		self.applied_A = Expression(('0','x[0]*amp*sin(2.0*pi*freq*t)','0') , degree=1, amp=B_amp, freq=nu, t=0, domain=self.mesh)
		bc.append( DirichletBC(self.space_em_glob.sub(1), self.applied_A, self.facet_ids, cfg['boundary']) ) 
		bc.append( DirichletBC(self.space_em_glob.sub(0), 0., self.facet_ids, cfg['boundary']) ) 
		...
		self.varsigma = AssignMaterialCoefficients(self.mesh, self.cell_ids, [varsigma_a, varsigma_e], mat_marking)
		self.pi = AssignMaterialCoefficients(self.mesh, self.cell_ids, [pi_a, pi_e], mat_marking)
	...
	def solve_em(self):
		...
		self.Bstrength = max(self.B_glob.vector().array())
		#print 'max E field ', max(self.E_glob.vector().array()), 'max B field ', max(self.B_glob.vector().array())
...
def output(t):
	...
	max_disp = max(u_.vector().array())
	#from IPython import embed ; embed()
	Bstrength = EM_prob.Bstrength  #z component of B field in the middle of the gel
	print 'magnetic flux: ', Bstrength, ' T,  tip displacement: ', max_disp, ' m'
	data1.append( [ t, Bstrength,  ])
	numpy.savetxt( pwd+'_time_disp.csv', numpy.array(data1) , delimiter=';' )
...
while t<tMax:
	...
	EM_prob.applied_A.amp = B_amp*t/tMax
